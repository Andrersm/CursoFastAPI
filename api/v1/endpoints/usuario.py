...
from schemas.usuario_schema import (UsuarioSchemaBase, UsuarioSchemaCreate,
                                    UsuariosSchemaArtigos, UsuarioSchemaUp)
...
async def post_usuario(usuario: UsuarioSchemaCreate,
    ...
    async with db as session:
        ...
        try:
            ...
            raise HTTPException(detail="Já tem esse email no banco de dados PATETA",
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    if not usuario:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Dados incorretos')
